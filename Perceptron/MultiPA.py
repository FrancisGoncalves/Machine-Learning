# ------------------------------------------- WSU - CPT_S 570 - HW1 -----------------------------------------
# --------------------- Online Multi-Class Classifier Learning Algorithm - Passive-Agressive -------------------
# ----------------------------------------- Author: Francisco Goncalves ------------------------------------------
#
# This code is a function that runs the Passive Agressive algorithm for Multi-Class Classification and calculates
...
# OUTPUTS:
#   accuracy_multiPA_training - Accuracy on the training data per iteration
#   accuracy_multiPA_testing - Accuracy on the testing data per iteration
#   mistakes_multi_PA - Mistakes per training iteration
#
from loadingData import D, E
import numpy as np

# Defining the function to be called in the main script
def MultiPA(T, accuracy_trigger):
    if accuracy_trigger == 1:
        print("Please wait a few moments while accuracy for Multi-Class Classification PA is computed...\n")
    else:
        print("Please wait a few moments while Online Learning curve for Multi-Class Classification PA is computed...\n")
    ...
    mistakes_multi_PA = np.zeros((1, T))
    accuracy_multiPA_training = np.zeros((1, T))
    accuracy_multiPA_testing = np.zeros((1, T))
    # Learning algorithm
    for i in range(T):
        ...
        for k in range(D[0].shape[0]):
            ...
            if prediction != D[1][k]:
                ...
                # Updating learn rate 'tau'
                tau = (1 - (np.inner(w[0, D[1][k] * 784:784 * (D[1][k] + 1)], D[0][k])-np.inner(w[0, prediction * 784:784 * (prediction + 1)], D[0][k])))/((np.linalg.norm(F(D[0][k], D[1][k]) - F(D[0][k], prediction)))**2)
                # Updating weight vector
                w = w + tau * (F(D[0][k], D[1][k]) - F(D[0][k], prediction))
        # Storing number of mistakes for this iteration
        mistakes_multi_PA[0, i] = training_mistakes
        ...
        if accuracy_trigger == 1:
            ...
            # this iteration
            accuracy_multiPA_training[0, i] = correct_training/60000
            # Initializing number of correct predicted labels
            correct_testing = 0
            ...
            # this iteration
            accuracy_multiPA_testing[0, i] = correct_testing/10000
    return accuracy_multiPA_training, accuracy_multiPA_testing, mistakes_multi_PA
